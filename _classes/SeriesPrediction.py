...
class StockPredictionNN(object): #aka CrystalBall
	...
	def LoadData(self, sourceDF:pandas.DataFrame, window_size:int=7, prediction_target_days:int=5, UseLSTM:bool=True, SourceFieldList:list=None, batch_size=32, train_test_split=.90):
		...
		if SourceFieldList == None: 
			sourceDF['CurrentAveragePrice'] = round((sourceDF['Low'] + sourceDF['High'] + sourceDF['Open'] + sourceDF['Close'])/4, RoundingPrecision)
			sourceDF = sourceDF[['CurrentAveragePrice']]
			#I don't need to shift the target dates because I am comparing on index not the date
			#self.targetDF['FuturePrice_Actual'] = sourceDF['CurrentAveragePrice'].shift(-prediction_target_days)
			self.targetDF['FuturePrice_Actual'] = sourceDF['CurrentAveragePrice'] 
		else:
			if UseLSTM:
				self.targetDF['FuturePrice_Actual'] = round((sourceDF['Low'] + sourceDF['High'] + sourceDF['Open'] + sourceDF['Close'])/4, RoundingPrecision)
			else:
				sourceDF = sourceDF[SourceFieldList].copy()
				self.targetDF = sourceDF.shift(-prediction_target_days).copy() #CNN is going to predict an image in the same shape as the original
		...
		for i in range(epochs):
			...
			while(ii + self.batch_size) <= len(self.X_train):
				...
				for iii in range(self.batch_size):
					...
					self.predictionDF.loc[rowIndex,'FuturePrice_Predicted'] = o[iii]
				ii += self.batch_size
			if (i % 10) == 0:
	...
	def PredictLSTM(self, epochs=100, learning_rate=0.001, dropout_rate=0.8, gradient_clip_margin=4, session = None):
		...
		while i + self.batch_size <= len(self.X_test):   
			...
			for iii in range(self.batch_size):
				...
				self.predictionDF.loc[rowIndex,'FuturePrice_Predicted'] = o[0][iii]
			i += self.batch_size
		session.close()
		print('Training session closed')
	...
	def GetTrainingResults(self, includeTrainingTargets:bool = False, includeAccuracy:bool = False):
		if includeTrainingTargets:
			...
			if includeAccuracy:
				if self.targetDF.shape[1] == 1:
					r['PercentageDeviation'] = abs((r['FuturePrice_Actual']-r['FuturePrice_Predicted'])/r['FuturePrice_Actual'])
				elif self.targetDF.shape[1] == 4:
					r['FuturePrice_Actual'] = (r['Open'] + r['High'] + r['Low'] + r['Close'])/4
					r['FuturePrice_Predicted'] = (r['Open_Predicted'] + r['High_Predicted'] + r['Low_Predicted'] + r['Close_Predicted'])/4
					r['PercentageDeviation'] = abs((r['FuturePrice_Actual']-r['FuturePrice_Predicted'])/r['FuturePrice_Actual'])
