...
#-------------------------------------------- Global settings -----------------------------------------------
use_database = PTADatabase().database_configured
...
#  ----------------------------------------------------  Model Builds  -----------------------------------------------------------------
    ...
    def _Build_Simple_Model(self):
        ...
        inputs_joined = keras.ops.concatenate(inputs, axis=-1)
    ...
    def _Build_LSTM_Model(self):
        ...
        lstm_units = 256
        inputs = self._InputLayer()
        x = keras.ops.concatenate(inputs, axis=-1)
    ...
    def _Build_BiLSTM_Model(self):
        lstm_units = 256
        inputs = self._InputLayer()
        x = Bidirectional(LSTM(units=lstm_units, return_sequences=True))(keras.ops.concatenate(inputs, axis=-1))
        #x = LayerNormalization()(x)
    ...
    def _Build_GRU_Model(self):
        inputs = self._InputLayer()
        inputs_joined = keras.ops.concatenate(inputs, axis=-1)
        x = GRU(units=128, return_sequences=True)(inputs_joined)
        x = LayerNormalization()(x)
    ...
    def _Build_Attention_Model(self):
        inputs = self._InputLayer()
        x = MultiHeadAttention(num_heads=8, key_dim=64)((keras.ops.concatenate(inputs, axis=-1)), (tf.concat(inputs, axis=-1)), (tf.concat(inputs, axis=-1)))
...
#  ----------------------------------------------------  Training / Prediction  -----------------------------------------------------------------
    ...
    def GetTrainingResults(self, include_target:bool = False, include_accuracy:bool=False, include_input:bool = False):
        if include_input:
            ...
            if include_accuracy and 'Average' in r.columns: 
                r['PercentageDeviation'] = np.where(r['Average'] != 0, abs((r['Average'] - r['Average_Predicted']) / (r['Average'] + 1e-8)),0)
    ...
    def PredictionResultsPlot(self, filename:str='', include_target:bool = False, include_accuracy:bool = False, days_to_plot:int=0):
        ...
        plotter = PlotHelper()
        plotter.PlotDataFrame(df=r, title=f"{self.ticker} Prediction Results",fileName=save_path, dpi=600)

# class StockPredictionNN(SeriesPredictionNN):
    # """
    # Predicts residuals relative to a persistence baseline instead of prices.
    # Baseline = current Average (or smoothed Average if you already applied it).

    # Final prediction:
        # y_hat = baseline + residual_hat
    # """

    # def _CustomTargetOperations(self):
        # """
        # Override target construction:
        # residual = future_average - current_average
        # """
        # # Ensure Average exists
        # if 'Average' not in self.targetDF.columns:
            # if 'High' in self.targetDF.columns and 'Low' in self.targetDF.columns:
                # self.targetDF['Average'] = (
                    # self.targetDF['High'] + self.targetDF['Low']
                # ) / 2
            # else:
                # raise ValueError("Average not found in targetDF")

        # # Align baseline with target rows
        # baseline = self.sourceDF['Average'].values
        # baseline = baseline[:len(self.targetDF)]

        # # Residual target
        # residual = self.targetDF['Average'].values - baseline

        # self.targetDF = pandas.DataFrame(
            # residual,
            # index=self.targetDF.index,
            # columns=['Residual']
        # )
    ...
    # def Predict(self, use_full_data_set: bool = False):
        # """
        # Predict residuals, then reconstruct prices:
            # prediction = baseline + residual_prediction
        # """
        # if self.model is None:
            # self.BuildModel()
        # if not self.batchesCreated:
            # self.MakeTrainTest()

        # # Prepare prediction dataframe
        # self.predictionDF = pandas.DataFrame(columns=['Average'], index=self.sourceDF.index)
        # self.predictionDF = self.predictionDF[self.prediction_target_days:]

        # # Extend index for forward prediction days
        # d = self.predictionDF.index[-1]
        # for i in range(self.prediction_target_days):
            # self.predictionDF.loc[d + BDay(i + 1), 'Average'] = numpy.nan
        # # Choose dataset
        # d_in = self.input_data if use_full_data_set else self.input_data_test
        # # Predict residuals
        # residual_preds = self.model.predict(d_in).astype(float)
        # baseline = self.sourceDF['Average'].values
        # start_index = len(self.predictionDF) - len(residual_preds)
        # for i in range(len(residual_preds)):
            # self.predictionDF.iloc[start_index + i, 0] = (baseline[start_index + i] + residual_preds[i][0])
        # print("Residual predictions complete.")
