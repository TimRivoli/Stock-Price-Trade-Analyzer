...
class PricingData:
	...
	def __init__(self, tker:str='Googl', dataFolderRoot:str=''):
		self.stockTicker = tker
	...
	def DownloadPriceData(self,daysToGoBack:int=0):
		...
		if useWebProxyServer:
			userName = 'Bill'
			password = 'test'
			proxyServer = '52.91.160.110'
			proxyPort = '80'
			proxy = webRequest.ProxyHandler({'https': r'http://' + userName + ':' + password + '@' + proxyServer + ':' + proxyPort})
	...
	def ConvertToPercentages(self):
		if self.pricesInPercentages:
			...
			for i in (range(1, self.historicalPrices.shape[0])):
				self.historicalPrices.iloc[i] = (self.historicalPrices.iloc[i]+1) * self.historicalPrices.iloc[i-1]
	...
	def PredictPrices(self, method:int=1, daysIntoFuture:int=1, NNTrainingEpochs:int=350):
		...
		if method < 3:
			...
			for i in range(0,daysIntoFuture): 	#Add new days to the end for crystal ball predictions
				...
				a = (self.pricePredictions['estLow'].tail(1).mean() + self.pricePredictions['estHigh'].tail(1).mean())/2
			...
			model.LoadData(self.historicalPrices, window_size=1, prediction_target_days=daysIntoFuture, UseLSTM=True, SourceFieldList=SourceFieldList, batch_size=10, train_test_split=.93)
			...
			self.pricePredictions = self.pricePredictions.rename(columns={'FuturePrice_Predicted':'estAverage'})
			deviation = self.historicalPrices['5DavDeviation'][-1]
			self.pricePredictions['estLow'] = self.pricePredictions['estAverage'] * (1 - deviation/2)
			self.pricePredictions['estHigh'] = self.pricePredictions['estAverage'] * (1 + deviation/2)
		elif method==4:	#Use CNN to predict prices
			model = StockPredictionNN()
			SourceFieldList = ['Low','High','Open','Close']
			model.LoadData(self.historicalPrices, window_size=daysIntoFuture*16, prediction_target_days=daysIntoFuture, UseLSTM=False, SourceFieldList=SourceFieldList, batch_size=32, train_test_split=.93)
...
class Portfolio:
	...
	def __init__(self, startDate:datetime, initialFunds:int=10000, _traunchSizeRequested:int=1000, portfolioName:str='', _verbose:bool=True):
		...
		self.dailyValue = pd.DataFrame([[startDate,initialFunds,0,initialFunds]], columns=list(['Date','_cashValue','AssetValue','TotalValue']))
...
class TradingModel:
	...
	def __init__(self, modelName:str, startingTicker:str, startDate:datetime, durationInYears:int, totalFunds:int, verbose:bool=False, dataFolderRoot: str=''):
		...
		if p.LoadHistory(True): 
			...
			self._stockTickerList.append(startingTicker)
	...
	def CloseModel(self, plotResults:bool=True, saveHistoryToFile:bool=True, folderName:str='data/trademodel/'):	
		...
		if plotResults: 
			self.PlotTradeHistoryAgainstHistoricalPrices(self.portfolio.tradeHistory, self.priceHistory[0].historicalPrices, self.modelName)
		return _cashValue
